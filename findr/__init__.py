...
# 
"""Python interface. See child: lib"""
...
class lib:
	...
	def __init__(self,path=None,loglv=6,rs=0,n_th=0):
		...
		n_th:	Maximum number of parallel threads. Default (0) indicates to use automatically determined number of cores (not always correct).
		"""
		self.lib=None
		from exceptions import ValueError, OSError
		from .auto import pkgname,version
		if type(rs) is not int or type(loglv) is not int or type(n_th) is not int:
		...
		for p in paths:
			try:
				...
				if((ln!=pkgname) or (lv!=pv)):
					...
					lib=None
					continue
				lib.lib_init(ctypes.c_ubyte(loglv),ctypes.c_ulong(rs),ctypes.c_ulong(n_th))
			except:
				lib=None
				continue
			break
